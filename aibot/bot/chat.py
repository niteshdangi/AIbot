...
def handle(query,request):
    ...
    if(e.keyCode == 13) {
		...
		if "this" in _query:
			_query = _query.replace("this",user[request.COOKIES['sessionID']]['this'])
		if _search:
			if _query == "":
				return ["Please Specify What you want to search!"]
			book_name = _query.replace("library","").strip()
			response = ["I cant find any book or Department with name %s " % (book_name)]
			if len(book_name.split(" "))>0:
				if not _book and not _ccode and not _dept:
					_book,_ccode,_dept=True,True,True
				match = difflib.get_close_matches(book_name.strip(),book_names)
				if len(match)>=1 and _book:
					for book in library:
						if book['name'].lower() == match[0].strip():
							response = ["This Book is available in %s Department on floor %s" % (book['dept'],str(book['floor'])),"<div class='btn msg-btn btn-primary' id=\"tmp_funct_con_%s\" onclick='tmp_funct_%s()'><script>function tmp_funct_%s(){$(\"#send\").removeAttr(\"disabled\");$(\"#chat-msg\").val(\"%%%% issue %s\");$(\"#send\").click();$(\"#tmp_funct_con_%s\").attr(\"disabled\",\"disabled\");}</script>Issue %s Book</div>" % (crr,crr,crr,book_name,crr,book_name.capitalize())]
							break
				match = difflib.get_close_matches(book_name,book_codes)
				if len(match)>=1 and _ccode:
					for book in library:
						if book['code'].lower() == match[0].strip():
							response = ["Books with this Course Code is available in %s Department on %s floor" % (book['dept'],str(book['floor']))]
							break
				match = difflib.get_close_matches(book_name,dept_names)
				if len(match)>=1 and _dept:
					for book in library:
						if book['dept'].lower() == match[0].strip():
							response = ["Books of this Department is available on %s floor" % (str(book['floor']))]
							break
			return response
		elif _issue:
			if _query == "":
				...
				for book in library:
					if book['name'].lower() == match[0].strip():
						if book not in user[request.COOKIES['sessionID']]['issued'] and book['available'] > 0:
							user[request.COOKIES['sessionID']]['issued'].append(book)
							if book in user[request.COOKIES['sessionID']]['returned']:
								user[request.COOKIES['sessionID']]['returned'].remove(book)
							book['available']-=1
							library[i] = book
							response = ["%s Book issued!"%(book["name"].capitalize()),"<div class='btn msg-btn btn-primary' id=\"tmp_funct_con_%s\" onclick='tmp_funct_%s()'><script>function tmp_funct_%s(){$(\"#send\").removeAttr(\"disabled\");$(\"#chat-msg\").val(\"%%%% return %s\");$(\"#send\").click();$(\"#tmp_funct_con_%s\").attr(\"disabled\",\"disabled\");}</script>Return this Book</div>"%(crr,crr,crr,book['name'],crr)]
						elif book['available'] == 0:
							response = ["This books is unavailable for issuing"]
						else:
							response = ["Book cant be issued as You have already issued this book"]
						break
					i+=1
			elif len(match)>1:
				iss = []
				for book in match:
					for i in library:
						if i['name'].lower() == book.lower():
							iss.append("<div style='margin:0;padding:0;' class='drt6t87ygh btn msg-btn' id=\"tmp_funct_con_%s\" onclick='tmp_funct_%s()'><script>function tmp_funct_%s(){$(\"#send\").removeAttr(\"disabled\");$(\"#chat-msg\").val(\" %%%% issue id %d\");$(\"#send\").click();$(\"#tmp_funct_con_%s\").attr(\"disabled\",\"disabled\");}</script>%s in %s (%s)</div>"%(crr+i["name"],crr+i["name"],crr+i["name"],i["id"],crr+i["name"],i["name"].capitalize(),i["dept"].capitalize(),i['code'].upper()))
				response = ["I found %d books with name '%s'"%(len(match),_query),"Click on the book name you want to issue:<ul><li>"+'<li>'.join(iss)+'</ul>']
			else:
			...
			_book_names = [x['name'].lower() for x in user[request.COOKIES['sessionID']]['issued']]
					...
					if book['name'].lower() == match[0].strip():
						if True:
							if book in user[request.COOKIES['sessionID']]['issued']:
								user[request.COOKIES['sessionID']]['issued'].remove(book)
							user[request.COOKIES['sessionID']]['returned'].append(book)
							book['available']+=1
							library[i] = book
							response = ["%s Book Returned!"%(book["name"].capitalize()),"<div class='btn msg-btn btn-primary' id=\"tmp_funct_con_%s\" onclick='tmp_funct_%s()'><script>function tmp_funct_%s(){$(\"#send\").removeAttr(\"disabled\");$(\"#chat-msg\").val(\"%%%% issue %s\");$(\"#send\").click();$(\"#tmp_funct_con_%s\").attr(\"disabled\",\"disabled\");}</script>Issue this book Again</div>"%(crr,crr,crr,book['name'],crr)]
						break
					i+=1
			else:
			...
			if dcount != 0 and _dept:
				response.append("There are %s books available in Department %s" % (str(dcount),_query))
			if bcount != 0 and _book:
				response.append("There are %s books available with name %s" % (str(bcount),_query))
			if ccount != 0 and _ccode:
				response.append("There are %s books available with Course Code %s" % (str(ccount),_query))
			if dcount == 0 and bcount == 0 and ccount == 0:
				response= ["I couldn't find anything with '%s'"%(_query)]
			if len(response) == 0 and "name" in query:
				response= ["I couldn't find any book with name '%s'"%(_query)]
			return response
		elif _query != "":
			qt = ""
			...
			if len(difflib.get_close_matches(_query,dept_names,cutoff=0.7))>0:
				qt+="/department" 
			response = ["What do you want to do with '%s'<br>I have this as a %s name"%(_query,qt)]
			if qt == "":
